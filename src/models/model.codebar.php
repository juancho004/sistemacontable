...
final class ModelCodeBar {
	...
	public function init()
	{
		#sleep(5);
		// format and code
		$code = (string)strtoupper($this->GenerateCode());
	...
	public function draw($filename = null) {
		...
		// display image
			...
			imagegif($img);
			#imagegif($img, 'Uploads/'.$this->code.'.gif');
		}
...
*/
	...
	public function initPrint($id,$code)
	{
...
public function drawPrint($filename = null ) {
		// check if GB library functions installed
		if(!function_exists("imagegif")) {
			return false;
		}
...
*/
	...
	{
		return $this->registerCode($this->RandomString());
	}
	...
	public function registerCode( $code )
	{
		...
		if( !$exist ){
			...
			$paramsTable 					= array();
			$paramsTable['code'] 			= $this->app->escape($code);
			#$paramsTable['registerDate'] 	= $this->app->escape(date("Y-m-d H:i:s"));
			$insert 						= $this->app['dbs']['mysql_silex']->insert($table ,$paramsTable);
			$this->code = $code;
			return $code;
	...
	public function RandomString($length=22,$uc=TRUE,$n=TRUE,$sc=FALSE)
	{
